...
public class Queen extends Piece {
    ...
    public boolean isLegitMove(int i0, int j0, int i1, int j1){
        int dI,dJ;
        // Moving Vertically, Horizontally or Diagonally but not Moving to the same place
        if((Math.abs(i1-i0) == Math.abs(j1-j0) && Math.abs(j1-j0) > 0)|| Math.abs(i1-i0) > 0 ^ Math.abs(j1-j0) > 0){
            ...
            for(int i=i0+dI,j=j0+dJ;i!=i1||j!=j1;i+=dI,j+=dJ){
                // A Piece is Blocking Queen's Path
        ...
        // Moving in Invalid Direction or Moving to the same place
        else{
            return false;
        }
    }	
